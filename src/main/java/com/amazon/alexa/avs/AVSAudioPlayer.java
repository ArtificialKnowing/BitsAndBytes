...
public class AVSAudioPlayer {
    ...
    private synchronized void play(final InputStream inpStream) {
        play(inpStream, false, false);
    }

    /**
     * Play a generic input stream through the default audio player
     */
    private synchronized void play(final InputStream inpStream, boolean block, boolean fromResource) {
            ...
            public void run() {
                synchronized (playLock) {
                    try {
                        ...
                        log.error("An error occurred while converting mp3 stream to wav format.", e);
                    }
                    catch (Exception e) {
